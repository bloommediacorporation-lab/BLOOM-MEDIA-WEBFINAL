<script lang="ts">
  ...
  $effect(() => {
      config.SIM_RESOLUTION = simResolution;
      if (gl) initFramebuffers();
  });
  $effect(() => {
      config.DYE_RESOLUTION = dyeResolution;
      if (gl) initFramebuffers();
  });
  $effect(() => {
      config.SHADING = shading;
      if (gl) updateKeywords();
  });
  // Update other config values reactively
  $effect(() => { config.DENSITY_DISSIPATION = densityDissipation; });
  $effect(() => { config.VELOCITY_DISSIPATION = velocityDissipation; });
  $effect(() => { config.PRESSURE = pressure; });
  $effect(() => { config.PRESSURE_ITERATIONS = pressureIterations; });
  $effect(() => { config.CURL = curl; });
  $effect(() => { config.SPLAT_RADIUS = splatRadius; });
  $effect(() => { config.SPLAT_FORCE = splatForce; });
  $effect(() => { config.COLOR_UPDATE_SPEED = colorUpdateSpeed; });
  $effect(() => { config.BACK_COLOR = backColor; });
  $effect(() => { config.TRANSPARENT = transparent; });
  ...
  let blit: (target: FBO | null, doClear?: boolean) => void;
...
<div class="pointer-events-none fixed top-0 left-0 z-50 size-full {className}">
  <canvas
    id="fluid"
    bind:this={canvas}
